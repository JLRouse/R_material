...
# Want a model that will work out likely average from numbers of runs
...
#Training least squares loss, measure of error
...
#Test Least squares loss, measure of error
...
# Looking at performance of test data, its not too bad
...
# Works in a bottom up fashion to cluster points
# Similar to k means but does not require the user to specify number of clusters before running the analysis
# Need to find distance, can use variable distance matrices but most commonly used are euclidean
...
# Dimensionality reduction tries to transform data into a 2D space summerising properties of the whole data set
